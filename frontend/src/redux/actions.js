...
export function fetchWorkDetails(id){
    return async dispatch => {
        commonAsyncDispatch(dispatch, FETCH_WORK_DETAILS, ()=> ports.getWorkDetails(id))
    }
}
export function clearhWorkDetails(){
    return {type: CLEAR_WORK_DETAILS}
}
export function fetchWorkImages(id){
    return async dispatch => {
        commonAsyncDispatch(dispatch, FETCH_WORK_IMAGES, ()=> ports.getWorkImages(id))
    }
}
